...
def create_go2_robot_with_lidar(scene, n_envs=1):
    ...
    robot = scene.add_entity(
        gs.morphs.URDF(
            file="urdf/go2/urdf/go2.urdf", pos=base_init_pos, quat=base_init_quat, fixed=False  # Allow robot to move
        )
    )
...
def run_lidar_simulation(scene, robot, lidar_sensor, base_init_pos, base_init_quat, n_envs=1, num_steps=200):
    ...
    if n_envs > 1:
        ...
        for env_idx in range(n_envs):
            ...
            print(
                f"  Environment {env_idx}: Moved robot to {new_positions[env_idx].cpu().numpy()} with yaw {env_idx * 0.5:.2f} rad"
            )
    ...
    print(
        f"  - Total rays per scan: {lidar_sensors[0].config['n_scan_lines'] * lidar_sensors[0].config['n_points_per_line']}"
    )
            ...
            if step % lidar_update_interval == 0:
                ...
                for env_idx in range(n_envs):
                    ...
                    if hit_points is not None and hit_distances is not None:
                        ...
                        n_hits = valid_hits.sum()
                        total_rays = (
                            lidar_sensors[0].config["n_scan_lines"] * lidar_sensors[0].config["n_points_per_line"]
                        )
                        ...
                        if n_hits > 0:
                            ...
                            if step % vis_update_interval == 0:
                                ...
                                current_point_cloud_nodes[env_idx] = scene.draw_debug_spheres(
                                    valid_points, radius=sphere_radius, color=color
                                )
                        else:
                            env_prefix = f"Env{env_idx}" if n_envs > 1 else ""
                            if env_idx == 0:  # Only show timing for first environment
                                print(
                                    f"Step {step:3d} {env_prefix}: LiDAR read time: {read_time:5.1f}ms, No hits detected"
                                )
                            else:
            ...
            if step % 10 == 0 and step > 0:  # Update robot positions every 10 steps
                ...
                for env_idx in range(n_envs):
                    ...
                    if step % 50 == 0:  # Print less frequently to avoid spam
                        print(
                            f"Step {step} Env{env_idx}: Moved robot to [{new_x:.2f}, {new_y:.2f}, {new_z:.2f}], yaw={np.degrees(yaw):.1f}°"
                        )
